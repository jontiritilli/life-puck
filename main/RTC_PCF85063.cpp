...
void PCF85063_Set_Alarm(datetime_t time)
{
	   ...
	   uint8_t buf[5];
	   buf[0] = (uint8_t)(((uint8_t)decToBcd(time.second)) & ((uint8_t)(~(uint8_t)RTC_ALARM)));
	   buf[1] = (uint8_t)(((uint8_t)decToBcd(time.minute)) & ((uint8_t)(~(uint8_t)RTC_ALARM)));
	   buf[2] = (uint8_t)(((uint8_t)decToBcd(time.hour)) & ((uint8_t)(~(uint8_t)RTC_ALARM)));
	   // buf[3] = (uint8_t)(decToBcd(time.day) & (~RTC_ALARM));
	   // buf[4] = (uint8_t)(decToBcd(time.dotw) & (~RTC_ALARM));
	   buf[3] = RTC_ALARM;   // disable day
	   buf[4] = RTC_ALARM;   // disable weekday
